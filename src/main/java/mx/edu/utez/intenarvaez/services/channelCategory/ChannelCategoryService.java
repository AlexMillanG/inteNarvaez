...
public class ChannelCategoryService {
    ...
    public ResponseEntity<ApiResponse> findAllCategoryChannel() {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(null, HttpStatus.INTERNAL_SERVER_ERROR, "error de solicitud", true));
        }
    }
    ...
    public ResponseEntity<ApiResponse> updateCategoryChannel(ChannelCategoryBean categoryBean) {
        try {
            ...
            logger.error("Error al consultar la info: ", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(null, HttpStatus.INTERNAL_SERVER_ERROR, "Ocurrió un error ", true));
        }
    ...
    public ResponseEntity<ApiResponse> findOneCategoryChannel(Long id) {
        ...
        try {
            if (id == null || id <= 0) {
                return ResponseEntity.badRequest().body(new ApiResponse(null, HttpStatus.BAD_REQUEST, "El id no debe ser nulo", true));
            }
            Optional<ChannelCategoryBean> foundCategory = channelCategoryRepository.findById(id);

            if (foundCategory.isEmpty()) {
                return ResponseEntity.badRequest().body(new ApiResponse(null, HttpStatus.BAD_REQUEST, "La categoria no se encontró", true));
            }
            ...
            logger.error("Error en la busqueda de los datos: ", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<ApiResponse> findByUuid(String uuid) {
        try {
            ...
            if (foundCategory.isEmpty()) {
                return ResponseEntity.badRequest().body(new ApiResponse(null, HttpStatus.BAD_REQUEST, " No existe la categoria", true));
            }
            ...
            logger.error("Error al solicitar los datos: ", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(null, HttpStatus.INTERNAL_SERVER_ERROR, "Ocurrió un error al solicitar la info", true));
        }
    ...
    public ResponseEntity<ApiResponse> delete(Long id) {
        try {
            if (id == null || id <= 0) {
                return ResponseEntity.badRequest().body(new ApiResponse(null, HttpStatus.BAD_REQUEST, "El id esta llegando nulo", true));
            }
            Optional<ChannelCategoryBean> foundChannelCategory = channelCategoryRepository.findById(id);

            if (foundChannelCategory.isEmpty()) {
                return ResponseEntity.badRequest().body(new ApiResponse(null, HttpStatus.BAD_REQUEST, "La categoria no se alló", true));
            }
            ...
            logger.error("Error al consultar los datos: ", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(null, HttpStatus.INTERNAL_SERVER_ERROR, "Error al solicitar la información", true));
        }
